---
title: "RSTR_LuminexAnalysis_PP1 (ESAT-6/CFP-10)"
...
df_mean_list <- lapply(df_list_2, function(df) {
  ...
  filter(stimulation == "PP1") # Only select the PP1 stimulated samples for the RSTR sequel paper
...
df_ttest_mat_port <- df_ttest_mat_port %>%
  as.data.frame()%>%
  select("6.PP1", "12.PP1", "24.PP1", "48.PP1" )
```
...
Select the PP1 (ESAT-6/CFP-10) stimulated samples and Subset the original data by the selected time points calculated in "highest rows"

```{r}

df_RSTR_comb_test_pp1 <- df_RSTR_comb_test %>%
  filter(stimulation == "PP1") %>% #only interested in PP1
...
for (i in 1:nrow(highest_rows)) {
  ...
  matching_rows <- df_RSTR_comb_test_pp1 %>% 
  ...
  if (nrow(matching_rows) > 0) {
    ...
    df_to_plot[[i]] <- data.frame(matrix(nrow = 0, ncol = ncol(df_RSTR_comb_test_pp1)))
  }
}
